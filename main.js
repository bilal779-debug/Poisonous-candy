...
async function updateProfile() {
    ...
    try {
        if (newAvatarFile) {
            // Process and get Base64 Data URL
            finalAvatarDataUrl = await uploadAndResizeAvatar(newAvatarFile);
        }
...
function declareGameResult(loserPlayerId) {
    get(ref(db, `${room}/players`)) 
        .then(snap => {
            ...
            // Determine actual names for winner and loser
            if (isAIMode) {
            ...
            let message = "";
            // Construct the message to be stored in Firebase.
            // This message should be a neutral, third-person statement about the game outcome.
            // Example: "Bilal wins! (Opponent picked Bilal's poison)"
            // Example: "AI Opponent wins! (Bilal picked AI Opponent's poison)"

            // If the loser is the AI (Player 2) in AI mode (human won)
            if (isAIMode && loserPlayerId === "2") { 
                message = `${winnerName} wins! (AI picked its own poison)`; // winnerName is human's name
            } 
            // If the loser is the Human (Player 1) in AI mode (AI won)
            else if (isAIMode && loserPlayerId === "1") { 
                message = `${winnerName} wins! (You picked ${winnerName}'s poison)`; // winnerName is AI Opponent
            } 
            // Multiplayer mode
            else { 
            ...
            // Set result and turn in Firebase
            set(ref(db, `${room}/result`), message)
                .then(() => console.log("DECLARE GAME RESULT: Result set successfully in Firebase:", message))
                .catch(error => console.error("DECLARE GAME RESULT: Error setting result in Firebase:", error));
            
            set(ref(db, `${room}/turn`), "end")
                .then(() => console.log("DECLARE GAME RESULT: Turn set to 'end' successfully."))
                .catch(error => console.error("DECLARE GAME RESULT: Error setting turn to 'end':", error));
        })
        .catch(error => console.error("DECLARE GAME RESULT: Error fetching players data:", error));
}
...
function startGame() {
    ...
    onValue(ref(db, `${room}/result`), snap => { 
        ...
        if (res) {
            let title = "";
            let message = res; // This message is now correctly formatted from declareGameResult
            ...
            if (res.includes("wins!")) {
                ...
                if (winnerName === window.playerName || (isAIMode && winnerName.includes("AI Opponent") && player === "2")) {
                    ...
                    title = "You Win!";
                    // For the winning client, show THEIR poison candy (which the opponent picked)
                    if (poisoned[player] !== -1 && poisoned[player] !== null) {
                        poisonCandyEmoji = `Your poisonous candy was ${candyEmojis[poisoned[player] % 10]} (candy no. ${poisoned[player] + 1}).`;
                    }
                } else {
                    // Current client is the loser
                    title = "You Lose!";
                    // For the losing client, show OPPONENT'S poison candy (which they picked)
                    ...
                    if (poisoned[actualWinnerPlayerId] !== -1 && poisoned[actualWinnerPlayerId] !== null) {
                        poisonCandyEmoji = `Opponent's poisonous candy was ${candyEmojis[poisoned[actualWinnerPlayerId] % 10]} (candy no. ${poisoned[actualWinnerPlayerId] + 1}).`;
                    }
                }
            } else {
                title = "Game Over"; 
                // For draw or forfeit, message is already set correctly in declareGameResult
                // No poison info for draw/forfeit
            }
